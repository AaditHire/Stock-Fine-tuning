...
STAGE6C_REPORT = ROOT / "results" / "training" / "stage6c_qwen3_4b_stage5d_v1.json"
...
def test_stage6c_report_records_completed_balanced_run() -> None:
    report = json.loads(STAGE6C_REPORT.read_text(encoding="utf-8"))

    assert report["status"] == "completed"
    assert report["run_name"] == "stage6c-qwen3-4b-stage5d-v1"
    assert report["smoke_test"] is False
    assert report["dataset"]["train_examples"] == 900
    assert report["dataset"]["validation_examples"] == 450
    assert report["dataset"]["train_token_max"] <= 512
    assert report["dataset"]["validation_token_max"] <= 512
    assert report["trainer"]["effective_batch_size"] == 16
    assert report["trainer"]["learning_rate"] == 0.00005
    assert report["lora"]["rank"] == 8
    assert report["parameters"]["trainable"] == 16_515_072
    assert report["metrics"]["evaluation"]["eval_loss"] > 0
    assert report["metrics"]["peak_gpu_device_used_mib"] < 6144
    assert report["adapter"]["weights_sha256"] == (
